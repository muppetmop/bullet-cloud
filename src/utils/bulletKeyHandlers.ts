...
export const handleEnterKey = (
  ...
  // If at start of line with content after cursor
  if (pos === 0 && content.length > 0) {
    const beforeCursor = "";
    const afterCursor = content;
    
    // Update current bullet to be empty
    onUpdate(bullet.id, beforeCursor);
    
    // Create new bullet with remaining content
    ...
    // Keep existing behavior for other positions
